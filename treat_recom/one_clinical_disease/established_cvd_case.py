def established_cvd(hba1c_records,previous_state,med_dose_last_time):
    ...
    if(previous_state=="First time"):
        ...
        if(current_hba1c>=target_):
            ...
            if(("Metformin").upper() in med_dose_last_time and (("SGLT2i").upper() in med_dose_last_time or ("GLP1RA").upper() in med_dose_last_time) and len(med_dose_last_time)==2):
                proposed_med[("Metformin").upper()]=full_dose
                if(("SGLT2i").upper() in med_dose_last_time):
